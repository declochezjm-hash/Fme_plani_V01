...
function withComputedPortOffsets(ports: Omit<PortDef, "offsetY">[]): PortDef[] {
	...
	return ports.map((port) => {
		const index = (port.side === "left" ? leftPorts : rightPorts).findIndex(
...
export function getNodeAttributes(node: EtlPipelineNode): NodeAttribute[] {
	...
	if (node.type === "reader") {
		const rasterAttributes = node.config["rasterAttributes"];
		if (Array.isArray(rasterAttributes) && rasterAttributes.length > 0) {
			return rasterAttributes.map((attr: { name: string; type: string }) => ({
				name: attr.name,
				type: attr.type,
			}));
		}

		const rasterMeta = node.config["rasterMeta"] as
			| Record<string, unknown>
			| undefined;
		if (node.config["format"] === "raster" && rasterMeta) {
			return Object.entries(rasterMeta).map(([name, value]) => ({
				name,
		...
		for (const feature of inline?.features?.slice(0, 5) ?? []) {
			for (const key of Object.keys(feature.properties ?? {})) {
				keys.add(key);
			}
		}
		for (const name of keys) {
			attributes.push({ name, type: "string" });
		}
		if (!attributes.some((attr) => attr.name === "geom")) {
...
export function getNodeHeight(node: EtlPipelineNode): number {
	const portCount = Math.max(
		getNodePorts(node).length,
		getNodeAttributes(node).length,
	);
...
export function getPortPosition(
	...
	const sidePorts = getNodePorts(node).filter(
		(item) => item.side === port.side,
	);
	...
	const x =
		port.side === "left" ? node.position.x : node.position.x + NODE_WIDTH;
...
export function defaultPortForConnection(
	...
	return (
		ports.find((port) => port.id === "input") ??
		ports.find((port) => port.side === "left") ??
		null
	);
}

export function resolvePortById(
	node: EtlPipelineNode,
	portId?: string,
): PortDef | null {
	const ports = getNodePorts(node);
...
export function resolveEdgePorts(
	...
	if (!sourcePort || !targetPort) {
		throw new Error("Impossible de résoudre les ports de connexion.");
	}
...
function findPortAtPoint(
	...
	for (const node of nodes) {
		for (const port of getNodePorts(node).filter(
			(item) => item.side === side,
		)) {
