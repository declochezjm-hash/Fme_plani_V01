...
export async function executePipeline(
  ...
  for (const nodeId of order) {
    ...
    const input = resolveInput(node, pipeline, outputs);
    if (!input) {
  ...
  for (const [id, dataset] of outputs.entries()) {
    intermediates[id] = dataset;
  }

  return {
    output: finalOutput,
    intermediates,
    metrics: { rowsRead, rowsWritten, durationMs, nodeResults },
    logs,
  };
}
...
function topologicalSort(pipeline: EtlPipelineJson): string[] {
